...
2. **只有写了多种写法的字才有变体**。当前 1,463 个字符里 **926 个有多种写法**，其余只有一种。
   变体数量不是均匀的：出现越多的字越值得多写几种（见 `scripts/make_write_list.py` 的字频分档）。
3. **★ PUA 码点不稳定 —— 每次 rebuild 都会变**。分配规则是「按字符码点升序，从 `U+E000` 起连续编号」，
   所以**任何一个字根增删一张 PNG，排在它后面的所有字的 PUA 都会被顶得前移或后移**。
   > 实例（2026-09-17）：「上」删掉一张错图后，`这` 的变体从 `U+E64C/E64D/E64E` 变成 `U+E64B/E64C/E64D`。
   **后果**：rebuild 之前生成的**渲染副本.txt 全部失效** —— 里面的 PUA 码点会被渲成别的字。
   **规矩**：`variant_map.json` 与其对应的 `渲染副本.txt` **必须同源同版本**；
   每次重 build 字体后，副本都要**重新生成**。`variant_map.json` 里记了 `base_ttf` / `images_dir`
   可用于核对来源，但**没有内容指纹** —— 换字体后务必重新跑 `make_variant_text.py`，不要复用旧副本。
...
| 事实 | 数据 |
|---|---|
| **base 字体 advance 统一 = unitsPerEm** | 全部字形 advance = **2048**（CJK 等宽方格），`lsb == xMin`。给新字形定 advance 时必须与主字形完全一致，否则含变体的段落会比纯主字形排得更紧 |
| **字形定位规则 = 整张画布线性映射** | 源图**整张画布**等比映射到 **0.9 em** 并居中：<br>`font_x = 102.4 + src_x × 3.6`<br>`font_y = 102.4 + (512 − src_y) × 3.6`（y 轴翻转）<br>（`3.6 = 0.9 × 2048 / 512`，`102.4 = (2048 − 0.9×2048)/2`）<br>该规则在 **191/191** 个字形上完全命中（±3 单位，含贴在角落的标点）。<br>★ 因为是「整张画布」映射，变体 PNG 与主字形 PNG 用**同一个变换**即天然对齐，**不需要**按墨迹 bbox 对齐（bbox 对齐遇到贴角标点会错位） |
| **字体里的字形比源图小 19%** | 源图映射到 0.9 em，面积比 = 0.9² = **0.81**。实测主字形墨迹/源图墨迹 = **0.807**，与我的矢量化结果（0.807）**完全一致** —— 即变体质量与 FontLab 那套基准流程相当 |
...
## 7. 已知 bug 与修复（必读，否则 PUA 渲染会反色）

### Bug 2026-09-15：PUA 字形渲染反色

**症状**：PUA 字形渲染出来是"白字黑底"（与主字形相反），且 mask 占满整个 em box。

**根因**：脚本处理素材 PNG 时，**没区分白底黑字 vs 黑底白字**，直接用 `bw = (a < 128)` 把"暗像素"当笔画。`data/sessions/all_characters/` 里两种格式的 PNG 都有 —— 黑底白字的图被错判后，cv2.findContours 找到 4 条轮廓，**第一条就是整个 512×512 画布边框**（"暗像素=笔画"意味着"画布整片都是笔画"）。PIL 渲染时把整个 em 框涂黑，真实字形变成"洞" → 视觉上反色。

**修复**（`scripts/build_variant_ttf.py:163-171`）：

```python
if a.mean() > 127:
    bw = (a < 128).astype(np.uint8)         # 白底黑字：暗的是笔画
else:
    bw = (a >= 128).astype(np.uint8)        # 黑底白字：亮的是笔画
# 不需要翻转：上面已经按"笔画=少数像素"的方向选 bw
# cv2.findContours 在错向 ink 下会把整张画布当笔画 (bw=1 占 80%+),
# 但我们用 "bw=1 占少数 = 笔画" 的天然属性即可
```

**修复后 cv2 找到的轮廓数**：
- 错向（暗=笔画，且原图黑底白字）：4 条，第一条 bbox=(0,0,512,512) ← 整张画布
- 正确（亮=笔画）：3 条，bbox 都在真实笔迹范围（U+E000: (660,283,1380,1773)）✅

**经验教训**：
- 任何对光栅位图做"轮廓提取"的步骤，**必须先识别 ink 方向**（黑底白字 vs 白底黑字），不能假设
- 同样的陷阱也在 `build_personal_ttf.py` 里存在，但那边**自带 auto 检测**（`util/lr_sched.py` 风格的 ink='auto' 分支），所以没翻车；本脚本最初拷过来时漏了这步
- 不需要写"占多数翻转"逻辑（笔画本就只占图像少数，逻辑上无需翻转）

### Bug 2026-09-15 之前：笔画方向 / bbox 设置

之前还修过两个相关 bug（详见 git log）：
1. **bbox 占满 em box**：早期版本用"墨迹 bbox 对齐"导致贴角标点被挪到中间 → 改为"整张画布线性映射"
2. **advance 逐字不同**：base 字体旧版 advance = 墨迹宽 + 22 侧边距 → 改为统一 2048
3. **lsb 恒为 0**：导致 PIL 渲染时字形整体左移 xMin → 改为 lsb = xMin
